...
public class DrawingService extends Service {
	...
	public void SendMsgToService(int state, int io) {
		Log.d("jack.chen", "drawingservice.java SendMsgToService state="
				+ state + " io=" + io);
		// m_IGpioControl =
		// IGpioControl.Stub.asInterface(ServiceManager.getService("gpio"));
		// try
		// {
	...
	private void hideNotifications() throws ClassNotFoundException,
	...
	Handler touch_SendMsg = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			Log.d("jack.chen", "DrawingService.java touch_SendMsg = "
					+ msg.what);
			try {
				switch (msg.what) {
				case 0:// hide
					if (uIController != null) {
						Log.d("jack.chen",
								"DrawingService.java uIController != null");
				...
				case 1:
					if (uIController != null) {
						...
						((com.yongfu.floatwindow.circlemenu.CircleImageView) uIController
								.getMainLayout().findViewById(R.id.mainButton))
								.setImageResource(R.drawable.edit_unsel);
						uIController.ShowNavWindow();
						Log.i("pizhu",
								"touch_SendMsg 1  ShowNavWindow  StartTimer(50000);");
	...
	private void onIntentCommandStart() {
		Log.d("jack.chen", "drawingservice.java onIntentCommandStart() uIController:"+uIController);
		if (this.uIController == null) {
	...
	private void onIntentCommandToggle() {
		...
		Log.d("jack.chen",
				"drawingservice.java onIntentCommandToggle() ->onIntentCommandHide()隐藏hideAll");
		onIntentCommandHide();
	...
	private void setButtonListeners() {
		...
		this.buttonWireing.setAllButtons(this.uIController.getNavWindow(),
				this.uIController.getBrushEditLayout(),
				...
				this.uIController.getSaveFileLayout(), new Runnable() {
					public void run() {
						try {
							new IntentCreator(getApplicationContext())
									.getKillIntent().send();
							return;
	...
	/*
	 * Thread mThread = new Thread("InputDeviceReader") { public void run() {
	 * RawInputEvent ev = new RawInputEvent(); while (true) {//开始消息读取循环
	 * 
	 * { InputDevice di; //本地方法实现，读取用户输入事件 readEvent(ev); }
	 * 
	 * 
	 * } }
	 */
				...
				if (uIController != null) {
					if (uIController.getPaintStatus() == false
						...
						if (touch_flag == false && close_flag == false) {
							Log.d("jack.chen",
									"drawingservice.java TimerTask() touch_SendMsg.sendEmptyMessage(0) invisible ");
					...
					Log.d("jack.chen",
							"drawingservice.java TimerTask() uIController == null");
				}
			}
		};
	...
	public void onCreate() {
		// Toast.makeText(this,
		// "Released by POPDA ( Persian OPDA ) NimA79 - A.l.i", 1).show();
		super.onCreate();
		...
		// NotificationCreator localNotificationCreator = new
		// NotificationCreator(getApplicationContext());
		// startForeground(localNotificationCreator.getID(),
		// localNotificationCreator.getNotification());
	...
	private BroadcastReceiver DataReceiverBroadcastReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			Log.d("jack.chen", "drawingservice.java onReceive() action="
					+ intent.getAction());
			if (intent.getAction().equals(RECEIVE_ACTION)) {
				...
				case (int) (0x8B & 0x0ff):
					if (uIController != null)
						((TextView) uIController.getMainLayout().findViewById(
								R.id.temp)).setText(String.valueOf(temp) + "℃");
					break;
				}
			}
			...
			if (intent.getAction().equals(ShowCircle))// jack.chen 20150814 add
														// to show UI
			{
			...
			if (intent.getAction().equals(HideCircle)) {
				Log.d("jack.chen",
						"drawingservice.java onReceive() touch_SendMsg.sendEmptyMessage(0)");
	...
	private void setBackground(View paramView, boolean paramBoolean) {
		Log.d("jack.chen", "drawingservice.java setBackground() paramBoolean="
				+ paramBoolean);
		if (paramBoolean) {
	...
	private void SetDefaultOnPaint(){
		Log.d("jack.chen"," SetOnPaint()---");
		uIController.togglePaintWindow();
		((com.yongfu.floatwindow.circlemenu.CircleImageView) uIController
				.getMainLayout().findViewById(R.id.mainButton))
				.setImageResource(R.drawable.edit_new);
		data.setBoolValue(true);
	
		uIController.getNavWindow().restoreTransperancy(false);
		uIController.getNavWindow().setFocu_un();
	}
	public int onStartCommand(Intent paramIntent, int paramInt1, int paramInt2) {
		Log.d("jack.chen", "drawingservice.java onStartCommand() paramInt1="
				+ paramInt1 + " paramInt2=" + paramInt2);
		super.onStartCommand(paramIntent, paramInt1, paramInt2);
		// onIntentCommandStart();

		if ((paramIntent != null) && (paramIntent.getExtras() != null)) {
			String str = paramIntent.getExtras().getString(
					IntentCommands.command.toString());
	...
	public void switchCommand(IntentCommands paramIntentCommands) {
		Log.d("jack.chen", "DrawingService.java +switchCommand()="
				+ paramIntentCommands.toString());
		...
		do {
			...
			if (paramIntentCommands == IntentCommands.touch) {
				...
				if (close_flag) {
					Log.d("jack.chen",
							"DrawingService.java +switchCommand() touch_SendMsg.sendEmptyMessage(1)");
					touch_SendMsg.sendEmptyMessage(1);// visible
				} else {
					Log.d("jack.chen", "DrawingService.java close_flag="
							+ close_flag);
				}
				return;
			}
